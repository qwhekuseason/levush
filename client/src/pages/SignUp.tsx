...
export default function SignUp() {
  const { user, signUp, signInWithGoogle, enabled } = useAuth();
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (!enabled) {
        throw new Error('Sign up is only available when Firebase is configured.');
      }
    ...
    <div className="container-site flex min-h-[80vh] items-center justify-center py-16 bg-grain">
      ...
      >
        <div className="glass-panel rounded-[2rem] p-8 md:p-10 shadow-[0_12px_40px_rgba(0,0,0,0.03)] border border-ink-600/40">
          ...
          {!enabled && (
            <div className="mb-6 rounded-xl border border-red-200 bg-red-50 p-4 text-xs text-red-800">
              <p className="font-semibold">Sign Up Disabled</p>
              <p className="mt-1">
                Firebase is not configured. Please configure credentials to register new accounts.
              </p>
              <div className="mt-3">
                <Link to="/signin" className="underline font-bold">Go to Sign In</Link>
              </div>
            </div>
          )}
          ...
          <div className="mt-8 text-center text-xs text-bone/50">
            ...
            <Link to="/signin" className="link-underline font-semibold text-gold">
              Sign In
            </Link>
          </div>
        </div>
